...
This folder is the one source. The copies under `DeepTest\docs`, `UntangleIt\docs`, `Witness\docs`, and `complexity\docs` are mirrors that `scripts\release.ps1` refreshes on every release, so a document is edited here and nowhere else. `ROADMAP.md` at the root of this repository is a mirror of `toolkit\toolkit-roadmap.md` for the same reason.
...
| `toolkit\witness.md` | Witness, the toolkit's own instrumentation, a library from 1.0.9: why it exists, the instrumenter, the runtime, the loaders, the Vite plugin, the Playwright worker hook, what proves it, and where it lives. | You are touching how a JavaScript test run is measured. |
...
| File | What it is | Read it when |
|---|---|---|
| `handoff-2026-09-12.md` | Where every tree stood at the end of 2026-09-12: what was committed, what was built and not yet committed (1.0.9, Witness as its own library), the commands to run, the decisions of the day, and the map for the rest of the JavaScript slot. | Work is resuming after a break. |
